...
EXTERNAL_NAMES: frozenset[str] = frozenset(
    {
        # The control library's device class, named where a claim
        # explains why it is not the unit a claim is built from.
        "EpicsMotor",
        # caproto's own simulator and its beacon task, both named by the
...
PROSPECTIVE_NAMES: frozenset[str] = frozenset(
    {
        ...
        "test_decide_emits_x",
        "test_handler_works",
        # A method this Protocol had and lost, named so the docstring can
        # say what went and why. Requiring it to exist would undo the
        # removal it describes.
        "walk_began",
        # The Protocol the keeper seam replaced, named so the prose can say
        # the replacement was not a rename.
        "Recording",
    }
)
...
EXTERNAL_FILES: frozenset[str] = frozenset(
    {
        ...
        # a beamline, a base URL and a token.
        "conductor.toml",
    }
)
