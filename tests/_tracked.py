...
SOURCE_ROOT = PROJECT_ROOT / "src" / "conductor"
...
def tracked_source_files() -> frozenset[Path]:
    """Absolute paths to tracked `.py` files under `src/conductor`."""
    return frozenset(
        PROJECT_ROOT / line for line in _ls_files("src/conductor") if line.endswith(".py")
    )
