...
| | For | Example |
|---|---|---|
| `EXTERNAL_NAMES` | Real, defined outside this project | `EpicsMotor`, the control library's device class |
| `PROSPECTIVE_NAMES` | Real nowhere, deliberately | `walk_began`, a method this package removed |
| `EXTERNAL_FILES` | A real file this project does not hold | `conductor.toml`, which an operator writes |
...
project found sixteen of them, and two file citations that pointed
outside it.
...
**Scopes:** `claims`, `procedure`, `conduct`, `intake`, `seams`, `config`, and one per adapter (`epics`, `bluesky`, `keeper`). Repo-level ones are `repo`, `deps` and `ci`. Pick the dominant scope or omit it.
...
**For anything non-trivial, use a worktree.** `git worktree add ../conductor-<task> main`, work there, commit, and return. Two reasons:
